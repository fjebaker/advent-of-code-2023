...
(define (count-winning nums1 nums2)
  (let [(winning-nums (set/intersection nums1 nums2))]
    (length winning-nums)))
...
(define (parse/card line)
  (let* [(parts (string-split line ":"))
         ...
         (score (count-winning winning-nums card-nums))]
...
;; calculates the current card's score, and updates the tally on the cards we have
  ...
  (let loop [(cards cards)]
    (unless (null? cards)
      (let* [(current (car cards))
        ...
        ; count the card we originally have
        (vector-set! tally card-num (+ (vector-ref tally card-num) 1))
        (let [(factor (vector-ref tally card-num))]
...
(let [(lines (getlines "day04/input.txt"))]
  (let* [(cards (map parse/card lines))
         ...
         (counts (make-vector (length cards) 0))]
